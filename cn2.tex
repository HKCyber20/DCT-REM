...
\subsection{光声平面内伺服框架：基于DCT的伺服控制算法设计}
\label{sec:algorithm}
...
正交投影模型下，图像雅可比矩阵 $\mathbf{J}(x, y)$ 的推导不再依赖于场景深度 $Z$，这使得它在处理光声成像这种小视野、近距离的任务时表现出极高的数值稳定性 ：
根据光声探头的内参 $\alpha_u, \alpha_v$ 以及正交投影几何关系 ，像素点 $(x, y)$ 随探头速度 $\mathbf{v}_c$ 运动的映射矩阵 $\mathbf{J}(x, y) \in \mathbb{R}^{2 \times 6}$ 定义为：

\begin{equation}
\begin{aligned}
\mathbf{J}(x,y) &=
\begin{bmatrix}
\alpha_u & 0 & 0 & 0 & \alpha_u Z & -(y - v_0) \\
0 & \alpha_v & 0 & -\alpha_v Z & 0 & (x - u_0) 
\end{bmatrix} \\[1em]
\mathbf{v}_c &=
\begin{bmatrix}
...
\end{equation}
由于光声成像仅在焦平面 $Z=0$ 处获取特征 ，该矩阵进一步简化为：
\begin{equation}
    \mathbf{J}(x, y) = \begin{bmatrix} \alpha_u & 0 & 0 & 0 & 0 & -(y - v_0) \\ 0 & \alpha_v & 0 & 0 & 0 & (x - u_0) \end{bmatrix}
\end{equation}
...
\end{equation}
\begin{equation}
\dot{\boldsymbol{I}}(x,y) = \nabla \boldsymbol{I}(x,y) \cdot \dot{\mathbf{v}}_p = -\nabla I(x, y)^\top \mathbf{J}(x, y) \mathbf{v}_c,
\label{eq:dIdt}
\end{equation}

其中 $\nabla \boldsymbol{I} = \left[ \partial_x \boldsymbol{I}, \partial_y \boldsymbol{I}\right]^\top$ 表示空间强度梯度，$\dot{\mathbf{v}}_p$ 表示投影到像素 $(x,y)$ 的点的瞬时 3D 速度 \cite{chaumette2006visual}.
...
\begin{equation}
\dot{F}_{uv} = \underbrace{\left( \sum_{x,y} \phi_{uv}(x,y) \cdot [-\nabla I(x,y)^\top \mathbf{J}(x,y)] \right)}_{\mathbf{L}_{F_{uv}}} \mathbf{v}_c,
\label{eq:LFuv}
\end{equation}
其中 $\boldsymbol{L}_{F_{uv}} \in \mathbb{R}^{1 \times 6}$ 构成了系数 $\boldsymbol{F}_{uv}$ 的交互行向量，利用正交投影 $\mathbf{J}(x, y)$ 的稀疏特性，我们可以清晰地看到不同自由度之间的解耦关系.
...
其中面内平移分量面内平移分量 ($v_x, v_y$):
\begin{equation} 
L_{v_x} =\nabla_x \boldsymbol{F}_{uv}= -\alpha_u \sum \phi_{uv}(x,y) \frac{\partial I}{\partial x},
\end{equation} 

\begin{equation} 
L_{v_y} = \nabla_y \boldsymbol{F}_{uv}=-\alpha_v \sum \phi_{uv}(x,y) \frac{\partial I}{\partial y}，
\end{equation} 
面内旋转分量 ($\omega_z$):
\begin{equation} 
L_{\omega_z} = \nabla_{\omega_z} \boldsymbol{F}_{uv}=\sum \phi_{uv}(x,y) \left( \frac{\partial I}{\partial x}(y - v_0)
- \frac{\partial I}{\partial y}(x - u_0) \right).
\end{equation} 
对于面外解耦分量 ($v_z, \omega_x, \omega_y$)来说，由于 $\mathbf{J}(x,y)$ 在第 3, 4, 5 列均为 0，推导出：$L_{v_z} = 0, \quad L_{\omega_x} = 0, \quad L_{\omega_y} = 0$ 这证明了单一光声平面无法观测面外运动,在正交模型下，单个像素点对速度的观测秩仅为 3，通过 DCT 叠加 $K$ 个系数可保证面内控制的稳定性 .
...
\begin{equation}
    L_{global} = \begin{bmatrix} L_{PA(K \times 3)} & \mathbf{0}_{K \times 3} \\ \mathbf{0}_{3 \times 3} & L_{out(3 \times 3)} \end{bmatrix} \in \mathbb{R}^{(K+3) \times 6}.
\end{equation}

\begin{itemize}
    \item 光声面内子空间 $\mathcal{V}_{in}$（前 $K$ 行）：对应自由度 $\{v_x, v_y, \omega_z\}$，由 $K$ 个自适应 DCT 系数梯度组成.其在面外分量处取 0，体现了光声切片对面外位移的解耦特性.
    \item 面外子空间 $\mathcal{V}_{out}$（后 3 行）：对应自由度 $\{v_z, \omega_x, \omega_y\}$，由深度相机几何约束主导.由于正交投影模型在物理上屏蔽了 $v_z, \omega_x, \omega_y$ 对二维图像灰度分布的贡献，这在数学上保证了全局交互矩阵 $L_{global}$ 的非对角块项 $\frac{\partial \mathbf{s}}{\partial \mathbf{v}_{out}}$ 恒等于零.这种结构性解耦不仅简化了计算，更从根源上消除了多传感器反馈之间的非线性耦合干涉.
...
\end{equation}
代入控制律 $\mathbf{v}_{PA} = -\lambda_{PA} \mathbf{L}_{PA}^+ \mathbf{e}_{PA}$ 和 $\mathbf{v}_{out} = -\lambda_{out} \mathbf{e}_{out}$ ,且在正交投影下，$\mathbf{L}_{out}$ 通常简化为单位阵 $\mathbf{I}_3$：
\begin{equation}
    \dot{V} = -\lambda_{PA} \mathbf{e}_{PA}^\top (\mathbf{L}_{PA} \mathbf{L}_{PA}^+) \mathbf{e}_{PA} - \lambda_{out} \mathbf{e}_{out}^\top \mathbf{e}_{out}
\end{equation}
由于 $\mathbf{L}_{PA} \mathbf{L}_{PA}^+$ 为半正定矩阵且 $\lambda_{out} > 0$ ，可知 $\dot{V} \le 0$.可以看出，通过 DCT 系数的自适应选择以及深度相机提供的非共线法向量，确保了$L_{PA}$ 在 $K$ 维特征下的满秩性.这证明了系统误差在 Lyapunov 意义下是单调不增的，即系统具有局部渐近稳定性 .
